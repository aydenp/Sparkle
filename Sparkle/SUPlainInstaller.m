...
- (BOOL)performInstallationToURL:(NSURL *)installationURL fromUpdateAtURL:(NSURL *)newURL withHost:(SUHost *)host progressBlock:(nullable void(^)(double))progress error:(NSError * __autoreleasing *)error
{
    ...
    NSURL *tempNewDirectoryURL = [self.fileManager makeTemporaryDirectoryWithPreferredName:preferredName appropriateForDirectoryURL:installationDirectory error:error];
    ...
    if (![self.fileManager moveItemAtURL:newURL toURL:newTempURL error:error]) {
        SULog(SULogLevelError, @"Failed to move the new app from %@ to its temp directory at %@", newURL.path, newTempURL.path);
        [self.fileManager removeItemAtURL:tempNewDirectoryURL error:NULL];
        return NO;
    }
    ...
    if (![self.fileManager releaseItemFromQuarantineAtRootURL:newTempURL error:&quarantineError]) {
    ...
    if (![self.fileManager changeOwnerAndGroupOfItemAtRootURL:newTempURL toMatchURL:oldURL error:error]) {
    ...
    if (![self.fileManager updateModificationAndAccessTimeOfItemAtURL:newTempURL error:error]) {
    ...
    NSURL *tempOldDirectoryURL = [self.fileManager makeTemporaryDirectoryWithPreferredName:oldDestinationName appropriateForDirectoryURL:oldURLDirectory error:error];
    ...
    if (![self.fileManager moveItemAtURL:oldURL toURL:oldTempURL error:error]) {
        ...
        [self.fileManager removeItemAtURL:tempNewDirectoryURL error:NULL];
        [self.fileManager removeItemAtURL:tempOldDirectoryURL error:NULL];
    ...
    if (![self.fileManager moveItemAtURL:newTempURL toURL:installationURL error:error]) {
        ...
        [self.fileManager moveItemAtURL:oldTempURL toURL:oldURL error:NULL];
        [self.fileManager removeItemAtURL:tempOldDirectoryURL error:NULL];
    ...
    SUFileManager *constrainedFileManager = [self.fileManager fileManagerByPreservingAuthorizationRights];
...
{
    return [self performInstallationToURL:[NSURL fileURLWithPath:self.installationPath] fromUpdateAtURL:[NSURL fileURLWithPath:self.bundlePath] withHost:self.host progressBlock:cb error:error];
}
